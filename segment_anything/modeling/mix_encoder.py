...
class Mix_ImageEncoderViT(nn.Module):
    def __init__(
        self,
        ...
        self.med_feature_indexes = med_feature_indexes
        self.med_attn_matrix_indexes = med_attn_matrix_indexes
        self.attn_weight_indexes = attn_weight_indexes
        self.med_features = {}
        self.med_attn_matrix = {}
    ...
    def matirx_to_weight(self,attn_matrix):
        # attn_matrix： [B*12,1024,1024]
        # attn_weight: [B,1,32,32]
        attn_weight = attn_matrix.view(-1, 12, 1024, 1024)
        attn_weight = attn_weight.mean(dim=-3)
        attn_weight = attn_weight.sum(dim=-2)
        attn_weight = attn_weight.contiguous().view(-1, 1, 32, 32)
        attn_weight = (attn_weight + 1)/2
        return attn_weight


class Mix_EvimgEncoderViT(nn.Module):
    def __init__(
        self,
        med_feature_indexes=[2, 5, 8, 11],
        med_attn_matrix_indexes=[2, 5, 8, 11],
    ...
    def forward(self, evimgs,image_tokens):
        ...
        for i,blk in enumerate(self.blocks):
            # [N,32,32,768],[N*12,1024,1024]
            x,attn_matrix = blk(x)
            if i in self.med_feature_indexes:
                # [B,C,H,W]
                self.med_features["block_"+str(i)] = x.permute(0, 3, 1, 2)

        return self.med_features
